<template>
  <div>
    ...
    <full-page :options="options" @after-load="afterLoad" id="fullpage">
      <div class="section">
        <h1 class="title"> diverge <span class="color">\</span></h1>
        <div class="copy">
          <p><span class="color">This is a thought piece </span>about five challenges Small Media has identified through conversations with ethnic rights advocates living in Iran.</p>
      </div>
      </div>
        <div class="section">
          <h3>Section 3</h3>
...
export default {
  ...
  methods: {
    ...
    addSection: function(e) {
      ...
      $('#menu')
        .find('li')
        .last().before(`<li data-menuanchor="page${newSectionNumber}">
      ...
      if (this.options.sectionsColor.length < newSectionNumber) {
        this.options.sectionsColor.push(
          this.options.sectionsColor[newSectionNumber - this.options.sectionsColor.length - 1]
        );
      }
    },
